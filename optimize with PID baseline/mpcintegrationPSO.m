...
options = optimoptions('particleswarm', 'SwarmSize', 20, 'MaxIterations', 200, 'Display', 'iter', 'UseParallel', false); % 禁用并行计算
lb = [0, 0, 0, 0, 0, 0]; % 下边界
ub = [100, 100, 100, 100, 1, 1]; % 上边界

% 目标函数定义
objectiveFunction = @(params) objectiveFunctionPSO(params, A, B, Ts, pid_baseline);

% 设置随机种子
%rng(62);

% PSO优化
[params_opt, fval] = particleswarm(objectiveFunction, 6, lb, ub, options);
%%  保存优化结果
...
% 目标函数定义
function J = objectiveFunctionPSO(params, A, B, Ts, target_integral_abs)
    % 提取权重矩阵参数
    Q = diag(params(1:4));
    R = params(5);
    R_delta = params(6);
...
% 假设当前优化得到的变量是 params_opt, fval, Q_opt, R_opt, R_delta_opt
% 你可以使用以下命令将这些变量保存到MAT文件中
